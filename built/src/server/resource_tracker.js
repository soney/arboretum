...
(function (My) {
    ...
    proto._requestWillBeSent = function (resource) {
        var request = resource.request, id = resource.requestId, url = request.url;
    ...
    proto.getResource = function (url) {
        ...
        return promise.then(_.bind(function (responseBody) {
            var resourceInfo = this._responses[url], content = responseBody.content, mimeType;
            if (resourceInfo) {
    ...
    proto._doGetResource = function (url) {
        var frameId = this._getFrameId(), chrome = this._getChrome();
        /*

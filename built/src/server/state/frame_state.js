var _ = require('underscore'), util = require('util'), EventEmitter = require('events'), EventManager = require('../event_manager').EventManager, ResourceTracker = require('../resource_tracker').ResourceTracker, DOMState = require('./dom_state').DOMState;
...
(function (My) {
    ...
    proto._handleQueuedEvent = function (eventInfo) {
        var eventType = eventInfo.type, event = eventInfo.event, promise = eventInfo.promise;
    ...
    proto.childNodeRemoved = function (event) {
        if (this._isRefreshingRoot()) {
            ...
            var node = this._getWrappedDOMNodeWithID(event.nodeId), parentNode = this._getWrappedDOMNodeWithID(event.parentNodeId);
            if (node && parentNode) {
    ...
    proto.childNodeInserted = function (event) {
        if (this._isRefreshingRoot()) {
            ...
            if (parentNode) {
                var previousNode = event.previousNodeId > 0 ? this._getWrappedDOMNodeWithID(event.previousNodeId) : false, wrappedNode = this._getWrappedDOMNode(event.node, parentNode);

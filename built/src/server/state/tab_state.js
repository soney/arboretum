var _ = require('underscore'), util = require('util'), URL = require('url'), EventEmitter = require('events'), FrameState = require('./frame_state').FrameState;
...
(function (My) {
    ...
    proto._executionContextCreated = function (event) {
        var context = event.context, frameId = context.frameId, frame = this.getFrame(frameId);
        if (frame) {
    ...
    proto._onFrameAttached = function (frameInfo) {
        var frameId = frameInfo.frameId, parentFrameId = frameInfo.parentFrameId;
    ...
    proto._onFrameNavigated = function (frameInfo) {
        var frame = frameInfo.frame, frameId = frame.id, frameUrl = frame.url;
    ...
    proto._createFrame = function (frameInfo, parent) {
        var resources = frameInfo.resources, childFrames = frameInfo.childFrames, frame = frameInfo.frame, frameId = frame.id;
    ...
    proto._updateFrameOnEvents = function (frame) {
        ...
        if (pendingFrameEvents) {
            ...
            _.each(pendingFrameEvents, function (eventInfo) {
                var eventType = eventInfo.type, event = eventInfo.event;

var _ = require('underscore'), URL = require('url'), util = require('util'), EventEmitter = require('events'), path = require('path'), urlTransform = require('../url_transform').urlTransform, driver = require('../hack_driver/hack_driver');
...
(function (My) {
    ...
    */
    ...
    proto._initializeLongString = function () {
        var node = this._getNode(), nodeType = node.nodeType, nodeValue = node.nodeValue;
    ...
    proto.getAttributesMap = function (shadow) {
        var node = this._getNode(), tagName = this._getTagName(), tagTransform = urlTransform[tagName.toLowerCase()];
        attributes = node.attributes,
            rv = {};
        if (attributes) {
            var len = attributes.length, i = 0;
            while (i < len) {
                var name = attributes[i], value = attributes[i + 1], newValue = value;
                if (shadow) {
    ...
    proto.initialize = function () {
        ...
        var longStringInitialization = this._initializeLongString(), namespaceFetcher = this.updateNamespace();
    ...
    proto._setAttribute = function (name, value) {
        var node = this._getNode(), attributes = node.attributes, found = false;
        if (!attributes) {
    ...
    proto.getNodeValue = function () {
        var node = this._getNode(), parent = this.getParent();
    ...
    proto._getMatchedStyles = function () {
        var id = this.getId(), chrome = this._getChrome();
    ...
    proto._requestInlineStyle = function () {
        var node = this._getNode(), type = node.nodeType;
        if (type === 1) {
            var id = this.getId(), chrome = this._getChrome(), inlineStyle;
            return new Promise(_.bind(function (resolve, reject) {
    ...
    proto.print = function (level) {
        ...
        var node = this._getNode(), type = node.nodeType, id = node.nodeId;

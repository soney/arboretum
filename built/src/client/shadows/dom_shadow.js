...
var _ = require('underscore'), util = require('util'), EventEmitter = require('events'), NODE_CODE = require('../../utils/node_code');
...
var ShadowDOM = function (options) {
    this.options = _.extend({
        ...
        childFilterFunction: function (child) {
            var node = child._getNode(), nodeName = node.nodeName, nodeType = node.nodeType;
            if (nodeName === 'SCRIPT' ||
...
(function (My) {
    ...
    proto._childAdded = function (info) {
        if (!this._is_initialized) {
            ...
            var child = info.child, previousNode = info.previousNode, toAdd, addedAtIndex;
            ...
            if (previousNode) {
                var previousNodeId = previousNode.getId(), myChildren = this.children, len = myChildren.length, i = 0, child;
                while (i < len) {
                    child = myChildren[i];
            ...
            if (toAdd instanceof My) {
                var state = this._getState(), previousNodeId = false, node;
    ...
    proto._childRemoved = function (info) {
        if (!this._is_initialized) {
            ...
            var removedChild = info.child, removedChildId = removedChild.getId(), myChildren = this.children, len = myChildren.length, i = 0, child, wasRemoved;
            while (i < len) {
                child = myChildren[i];
...
*/
    ...
    proto._handleQueuedEvent = function (eventInfo) {
        var eventType = eventInfo.type, info = eventInfo.info;
    ...
    proto.serialize = function () {
        var tree = this.getTree(), node = tree._getNode(), parent = this.getParent(), parentId = parent ? parent.getId() : false;
        return {
            parentId: parentId,
            id: this.getId(),
            type: this._type,
    ...
    proto._initialize = function () {
        var tree = this.getTree(), node = this.getNode();

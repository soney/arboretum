var _ = require('underscore'), util = require('util'), EventEmitter = require('events'), ShadowDOM = require('./dom_shadow').ShadowDOM;
...
(function (My) {
    ...
    proto._initialize = function () {
        var domTree = this._getDomTree(), socket = this._getSocket();
    ...
    proto._updateShadowTree = function (destroy) {
        ...
        if (node) {
            var shadow = this._shadowTree = new ShadowDOM(_.extend({}, this.options, {
                tree: node,
                state: this,
                parent: false
            }));

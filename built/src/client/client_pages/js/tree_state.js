$.widget('arboretum.tree_state', {
    ...
    _serverReady: function (data) {
        ...
        if (data.type === DOCUMENT_NODE && data.children.length === 1 && data.children[0].name === 'HTML') {
    ...
    _valueUpdated: function (info) {
        var element = this.nodeMap[info.id], type = info.type, value = info.value;
        if (type === 'canvas') {
    ...
    _nodeInitialized: function (info) {
        var node = this.nodeMap[info.id], element = node.element;
    ...
    _childAdded: function (info) {
        var parentId = info.parentId, serializedChild = info.child, previousNodeId = info.previousChild, previousChild = this.nodeMap[previousNodeId], parent = this.nodeMap[parentId];
        if (parent) {
    ...
    _childRemoved: function (info) {
        var parentId = info.parentId, childId = info.childId, parent = this.nodeMap[parentId], child = this.nodeMap[childId];
        if (parent && child) {
    ...
    _childrenChanged: function (info) {
        var parentId = info.parentId, serializedChildren = info.children, parent = this.nodeMap[parentId];
        if (parent) {
    ...
    _valueChanged: function (info) {
        var nodeId = info.id, value = info.value, node = this.nodeMap[nodeId];
        if (node) {
    ...
    _attributesChanged: function (info) {
        var nodeId = info.id, attributes = info.attributes, inlineStyle = info.inlineStyle, node = this.nodeMap[nodeId];

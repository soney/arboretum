...
$.widget('arboretum.tree_node', {
    ...
    _getChildElement: function (child) {
        var childType = child.type, childElem;
    ...
    _postChildElementAdded: function (child, childElem) {
        var state = this.option('state'), childType = child.type;
    ...
    childAdded: function (child, previousChild) {
        var children = this.option('children'), childElem = this._getChildElement(child);
        if (childElem) {
            if (previousChild) {
                var childElements = this.element.children(), toAdddIndex = -1, toAddAfter;
    ...
    setAttributes: function (attributes, inlineStyle) {
        var previousAttributes = this.option('attributes'), previousInlineStyle = this.option('inlineStyle');
        this.option({
